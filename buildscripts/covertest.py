...
BASE_PKG = "github.com/docker/libentitlement"


def get_all_pkgs(buildtags):
...
def run_test_with_coverage(buildtags="", coverdir=".cover", pkgs=None, opts="", covermode="count"):
    ...
    """
    all_pkgs = get_all_pkgs(buildtags)
    all_pkgs.sort()
    ...
    for pkg in pkgs:
        cmd = base_cmd + ["-coverprofile",
                          os.path.join(
                              coverdir,
                              get_coverprofile_filename(pkg, buildtags)
                          ),
                          pkg]
        ...
        for line in app.stdout:
            ...
            if not line.startswith("warning: no packages being tested depend on github.com/docker/libentitlement"):
                line = line.replace(allpkg_string, "<all packages>").replace(
                    pkg,
                    pkg.ljust(longest_pkg_len)
                )
...
def parseArgs(args=None):
    ...
    parser.add_argument(
        "--coverdir",
        help="The coverage directory in which to put coverage files",
        default=".cover"
    )
    parser.add_argument(
        "--testopts",
        help="Options to pass for testing, such as -race or -v",
        default=""
    )
    parser.add_argument(
        "--pkgs",
        help="Specific packages to test, otherwise we test all the packages",
        default=""
    )
...
if __name__ == "__main__":
    ...
    run_test_with_coverage(coverdir=args.coverdir, buildtags=args.tags,
                           pkgs=pkgs, opts=args.testopts)
